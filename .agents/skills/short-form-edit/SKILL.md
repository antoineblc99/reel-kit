...
> Taken from Nate Herk's HyperFrames Student Kit (MIT). Adapted for reel-kit: the project = the reel folder (cloned from reel-kit), Nate's MOTION_PHILOSOPHY.md = `references/nate-MOTION_PHILOSOPHY.md`, transcription = `scripts/transcribe.py` (ElevenLabs Scribe by default, WhisperX local fallback), never ElevenLabs dubbing or generation; generation only as a last resort. The qa-*.mjs / integrate.mjs scripts cited here are not provided: use `npm run check`, `npm run preflight`, `npm run frames` instead.
...
1. Read the scoped workspace guide, `references/nate-MOTION_PHILOSOPHY.md`, and the HyperFrames
   and video-storytelling skills before composition work. Use the user's brief
   over aesthetic defaults. Keep all artifacts in `<dossier du reel>/`.

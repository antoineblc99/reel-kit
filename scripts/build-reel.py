        ...
        for j, it in enumerate(z.get("items", [])):
            ...
            if typ == "card":
                ...
                if it.get("value") is not None and it.get("count"):
                    tl.append(f'(function(){{ const el = document.getElementById("{iid}-num"); const o = {{ v: 0 }}; tl.to(o, {{ v: {it["value"]}, duration: {r3(it.get("count", 0.9))}, ease: "power2.out", onUpdate: () => {{ el.textContent = Math.round(o.v).toLocaleString("fr-FR").replace(/\u202f|\u00a0/g, " ") + el.dataset.suffix; }} }}, {rel(it.get("count_at", it.get("at", t0)))}); }})();')
                ...
                if it.get("value"):
                    tl.append(f'(function(){{ const el = document.getElementById("{iid}-t").childNodes[0]; const o = {{ v: 0 }}; tl.to(o, {{ v: {it["value"]}, duration: {r3(it.get("count", 0.8))}, ease: "power2.out", onUpdate: () => {{ el.textContent = Math.round(o.v).toLocaleString("fr-FR").replace(/\u202f|\u00a0/g, " ") + "{esc(it.get("suffix", " €"))}"; }} }}, {at}); }})();')
...
<html lang="fr"><head><meta charset="UTF-8" /><!-- generated by scripts/build-reel.py: edit storyboard.json, not this file --></head>
<body><template>
